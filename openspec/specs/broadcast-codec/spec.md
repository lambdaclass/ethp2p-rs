# broadcast-codec Specification

## Purpose

`broadcast-codec` defines how `ethp2p-rs` encodes, decodes, and frames
the protobuf wire messages of the broadcast layer. It vendors the
upstream `.proto` schemas byte-identically, generates Rust types via
`prost-build`, supplies length-prefixed framing helpers for BCAST and
SESS streams, manages the stream-opening selector exchange, lays out
CHUNK streams as `Selector || Header || raw_bytes`, and ships a
conformance corpus with byte-level golden tests. Subsequent slices
