# broadcast-rs-strategy Specification

## Purpose

`broadcast-rs-strategy` defines the Reed-Solomon erasure-coding strategy
for the ethp2p broadcast layer. It specifies origin encoding, the
synchronous per-chunk SHA-256 verification model, k-of-n reconstruction
with end-to-end message-hash integrity, the routing bitmap (shard
havelist) used as the RS routing-update payload, the emit planner that
orders shards by allocation count with Fibonacci-hashed tie-breaks, and
the configuration parameters from spec §8 governing all of the above.

This is the first concrete `Strategy` consumer of `broadcast-codec`'s
`rs.Preamble` and `rs.ChunkIdent` types. Slice 4 (engine) introduces
the `Strategy` trait that this spec realizes; until then the
implementation exposes the same surface via inherent methods on
`RsStrategy`. Future strategies (RLNC, KZG-authenticated) extend or
replace this capability while reusing the framework boundaries
established here.
## Requirements
