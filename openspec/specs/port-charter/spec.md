# port-charter Specification

## Purpose

`port-charter` records the foundational, non-runtime commitments that
constrain every other capability in `ethp2p-rs`: the spec source of
truth, the clean-room contribution policy, the wire-compatibility scope,
the dual MIT + Apache 2.0 license posture, and the seven-slice delivery
