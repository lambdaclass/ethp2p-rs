# cgo-fuzz-harness Specification

## Purpose

`cgo-fuzz-harness` defines the differential fuzzing infrastructure that
empirically validates `port-charter` Requirement 3 (wire-compatibility
scope). It specifies the cargo-fuzz integration layout, the C ABI the
`goref/` shim must export, the centralized unsafe FFI module that
consumes that ABI, the feature-gated build pipeline that links the
shim only when present, the sanity and differential fuzz target shapes,
and the shim-maintainer process boundary that operationalizes
`port-charter` Requirement 2 (clean-room contribution policy).

The harness is the only place in the repository where unsafe code or
upstream Go source is permitted, and only inside the explicitly named
modules (`fuzz/src/ffi.rs` and `fuzz/goref/`). Subsequent slices
extend the harness with new fuzz targets (e.g. RS strategy, engine
state machine) but the contract recorded here governs how those
targets bridge to the Go reference.
